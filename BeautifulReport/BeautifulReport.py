...
class ReportTestResult(unittest.TestResult):
    ...
    def get_all_result_info_tuple(self, test):
        ...
        :return:
        """
        class_name, method_name, method_doc = self.get_testcase_property(test)
        return tuple([class_name, method_name, method_doc,
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def report(self, description, filename=None, report_dir='.',
               log_path=None, theme='theme_default'):
        """
            生成测试报告,并放在当前运行路径下
    ...
    def output_report(self, theme):
        ...
        def render_template(params, template):
